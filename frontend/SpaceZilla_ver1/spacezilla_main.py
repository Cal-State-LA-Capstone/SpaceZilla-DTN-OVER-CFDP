...
class MainWindow:
    ...
    def open_add_contact(self):
        ...
        address_box = self.contact_edit_window.findChild(QLineEdit, "contact_address")
        if typed_address:
            name_box.setText(typed_address)
            address_box.setText(typed_address)
        button_box = self.contact_edit_window.findChild(QDialogButtonBox, "buttonBox")
        button_box.accepted.connect(lambda: self.save_contact(name_box, address_box))
        button_box.rejected.connect(self.contact_edit_window.close)
        self.contact_edit_window.show()

    # Validates and saves a new contact, then closes the edit window
    def save_contact(self, name_box, address_box):
        name = name_box.text().strip()
        address = address_box.text().strip()
        if not name or not address:
            return
        self.add_to_contact_list(name, address)
        self.contact_edit_window.close()
        self.destination_filter.clear()

    # Builds and adds a contact row widget to the contact list
    def add_to_contact_list(self, name, address):
        row_widget = QWidget()
        row_layout = QHBoxLayout(row_widget)
    ...
    # Opens the Edit Contact window, pre-populated with the contact's current info
    def open_edit_contact(self, name_label, address_label, row_widget):
        self.contact_edit_window = load_ui("Contact_Info_Edit.ui")
