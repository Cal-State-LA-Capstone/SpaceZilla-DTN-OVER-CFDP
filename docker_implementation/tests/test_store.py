...
from store.globals import load_settings, load_theme, save_settings
...
class TestGlobalSettings:
    ...
    def test_host_mount_consent_default(self):
        expected = False
        actual = GlobalSettings().host_mount_consent
        assert actual == expected

    def test_host_mount_consent_at_default(self):
        expected = None
        actual = GlobalSettings().host_mount_consent_at
        assert actual == expected
...
class TestLoadSettings:
    ...
    def test_ignores_unknown_keys(self):
        ...
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(
            json.dumps({"theme": "dark", "log_level": "DEBUG", "unknown": 42})
        )
        # Should not raise TypeError despite the unknown key.
        settings = load_settings()
...
class TestSaveSettings:
    ...
    def test_roundtrip_consent_granted(self):
        expected = GlobalSettings(
            host_mount_consent=True,
            host_mount_consent_at="2026-04-23T12:00:00",
        )
        save_settings(expected)
        actual = load_settings()
        assert actual == expected
    ...
    def test_persists_to_disk(self):
        save_settings(GlobalSettings(theme="dark", host_mount_consent=True))
        data = json.loads(settings_path().read_text())
        assert data["theme"] == "dark"
        assert data["host_mount_consent"] is True
