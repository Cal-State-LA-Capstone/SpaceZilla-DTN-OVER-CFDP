"""DEPRECATED — replaced by :mod:`backend.zmq_controller`.

This module wires up the legacy FastAPI/uvicorn HTTP IPC on the host.
It predates the per-container pyion adapter: :mod:`backend.pyion_adapter`
has to live in the same process tree as ``ionstart``, which is only
true once the backend is moved inside the Docker container. New code
should use :class:`backend.zmq_controller.ZmqController`.

Kept alongside the new controller so external imports keep working
during the transition — no behavioural changes.
